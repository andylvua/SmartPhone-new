...
int PDU::encodePDU(const char *recipient, const char *message, unsigned short csms, unsigned char numparts, unsigned char partnumber)
{
 ...
 // bool intl = *recipient == '+';
  ...
  if ((numparts + partnumber) == 0)
    ; // OK so far
  else
  {
    if (numparts == 0 || partnumber == 0)
  ...
  */
  ...
  // }
  // just write 00 for SCA
  tempbuf[smsOffset++] = 0;
  beginning = smsOffset;

  int pdutype = PDU_SMS_SUBMIT | 0b100000; // request status report
  if (numparts != 0)
    pdutype |= (1 << PDU_UDHI); // set UDH bit for concatenated message
  tempbuf[smsOffset++] = pdutype;
  tempbuf[smsOffset++] = csms; // message reference
...
//  memcpy(tempbuf, generalWorkBuff, length);
  ...
  generalWorkBuff[(length * 2)] = 0; // add end marker
